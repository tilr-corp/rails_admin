class Player
  ...
  #validates_uniqueness_of(:number, scope: :team, message: 'There is already a player with that number on this team')

  validates_each :name do |record, _attr, value|
    record.errors.add(:base, 'Player is cheating') if value.to_s =~ /on steroids/
  end

  has_one :in, :draft, type: :HAS_PLAYER, dependent: :destroy
  has_many :in, :comment, origin: :commentable
